...
function systemPrompt(): string {
  return [
    ...
    '5b) 情节音效（sfx）：分析每页 narration 正在讲的动作、场景与情绪，若可用声音强化就给出音效提示（每页 0-2 条，宁缺毋滥，没有就留空数组或省略）。type 只能取——动作/情绪声：laugh（大笑）、giggle（咯咯偷笑）、cry（哭泣/抽泣）、applause（鼓掌喝彩）、cheer（欢呼「耶」）、gasp（惊讶倒吸气）、sigh（叹气）、magic（魔法闪光/变身）、whoosh（快速移动/转场呼啸）、heartbeat（心跳——紧张害怕时）、yawn（打哈欠——困了）、snore（打呼噜——入睡）；动物叫：cat（猫）、dog（狗）、rooster（公鸡打鸣）、duck（鸭子）、frog（青蛙）、cow（奶牛）；自然/物件：footsteps（脚步）、door（开门关门）、knock（敲门）、bell（铃铛）、thunder（雷声）、birds（鸟鸣）、water（流水戏水）、waves（海浪）、fire（篝火噼啪）、clock（时钟滴答）、phone（电话铃）、balloon（气球爆）、page_turn（翻书页）；渲染：drum_roll（鼓点悬念）、fanfare（胜利号角）。',
    '   a. 触发条件：只在旁白字面讲到或明确演到对应声音时才加——旁白出现「小猫喵了一声」才配 cat，旁白说「大家鼓起掌来」才配 applause。情绪类音效更要有情节依据：欢乐高潮且有围观/庆祝情节才配 applause/cheer（不要仅因为页情绪是 joyful 就塞笑声）；神奇时刻配 magic；tense 波折页配 heartbeat 或 drum_roll（二选一，全篇至多一次）；战胜困难/愿望实现的胜利页配 fanfare（全篇至多一次）；睡前收尾页可配 yawn 或 snore（二选一）。cry 全篇至多一次；balloon（气球爆）是惊吓声，只在旁白真写到气球爆了才用。雨声/风声这类持续环境声交给 fx_hints.ambient 处理，不要重复放进 sfx。',
    ...
    '15) 输出前自查：通读全部 narration，逐对检查相邻两页是否衔接自然；再通读全部 scene_desc，确认时间、地点、道具前后连贯；核对每页 segments 拼接后与 narration 一致，并逐句检查角色对白——删掉上下文后单独读仍成立的对白是失败的对白，必须能看出是在回应前文、贴合该页情绪；最后逐条核对 sfx——每条都能在旁白文字里找到出处、at 与该词在旁白中的位置相符，情绪渲染类音效（applause/cheer/fanfare/heartbeat/cry）未超量。不通过就先在脑中修改再输出。',
  ].join('\n');
}
