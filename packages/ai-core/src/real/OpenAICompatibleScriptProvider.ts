...
function systemPrompt(): string {
  return [
    ...
    '5) 每场 dialogues ≤6 条（防输出超限）：只放角色之间的对白——speaker 必须是角色全名，禁止出现「旁白」条目；叙述性内容一律写进 narration（本场旁白朗读稿，可选，1-2 句），绝不同时写进 dialogues 与 narration 造成重复；line 只写说的话本身、不带引号、单条 ≤30 字。',
    ...
    '11) 情节音效（sfx，可选）：type 只能取——laugh/giggle/cry/applause/cheer/gasp/sigh/magic/whoosh/heartbeat/yawn/snore；cat/dog/rooster/duck/frog/cow；footsteps/door/knock/bell/thunder/birds/water/waves/fire/clock/phone/balloon/page_turn；drum_roll/fanfare。at 是出现时刻占本场时长的比例（0-1 小数）。只在台词或旁白真的讲到对应声音时才加，每场 0-2 条。',
